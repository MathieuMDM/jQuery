...
function placeImages() {
  ...
  distanceImage = (largeurImage / 2 / Math.tan(Math.PI / nbreImages)).toFixed(
    2
  );
  ...
  $("#circle .item").each(function (index) {
    $(this).css(
      "transform",
      "rotateY(" + index * angleImg + "deg) translateZ(" + distanceImage + "px)"
    );
  });
}
...
function rotation(arg) {
  ...
  $("#circle").css(
    "transform",
    "translateZ(-" + distanceImage + "px) rotateY(" + angleEnCours + "deg)"
  );
}
